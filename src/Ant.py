...
class Ant:
    ...
    def update(self, time, possibleDirs, dirWeights):
        ...
        if time % 5 == 0:
            if self.hasFood:
                self.direction = np.array([self.nestX - self.x, self.nestY - self.y])
                ...
                if sumPoids > 0:
                    self.direction = possibleDirs[np.random.choice(len(possibleDirs), p=dirWeights/np.sum(dirWeights))]
                else:
                    self.direction = vectRot(self.direction, np.random.uniform(-np.pi/4, np.pi/4))
