...
class World:
    ...
    def __init__(self, canvas, width, height, cellsX, cellsY):
        ...
        self.grille = np.array(
            [
                [
                    Cell(canvas, canvas.create_rectangle(x * self.cellW, y * self.cellH, (x+1) * self.cellW, (y+1) * self.cellH, outline=""))
        ...
        for nest in self.nests:
            for ant in nest.ants:
                ...
                if ant.hasFood:                    
                    ...
                    for food in self.food:
                        if sqrt((ant.x - food.x)**2 + (ant.y - food.y)**2) <= food.maxAmount and food.amount > 0:
                            food.decrease(1)
                            ant.hasFood = True
                
                if sqrt((ant.x - nest.x)**2 + (ant.y - nest.y)**2) <= nest.taille:
                    ant.hasFood = False
                    ant.resetStamina()
                ...
                for gX in range(-2, 3):
                    for gY in range(-2, 3):
                        ...
                        if angle < np.pi / 2 and angle > -np.pi / 2:
                            ...
                            dirWeights.append(self.grille[a[0], a[1]].pheromones[0])
